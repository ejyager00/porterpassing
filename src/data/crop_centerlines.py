...
def get_relevant_centerlines(centerlines, bounds):
    ...
    for polyline in polylines:
        new_points = []
        if is_in_bounds(polyline[0]):
            new_points.append(polyline[0])
        for i in range(1, len(polyline)):
            if is_in_bounds(polyline[i-1]) and is_in_bounds(polyline[i]):
                new_points.append(polyline[i])
            elif is_in_bounds(polyline[i-1]):
                new_points.append(boundary_intersect(polyline[i-1], polyline[i]))
            elif is_in_bounds(polyline[i]):
                new_points.append(boundary_intersect(polyline[i-1], polyline[i]))
                new_points.append(polyline[i])
        if len(new_points)>1:
